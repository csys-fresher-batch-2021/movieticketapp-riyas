...
<body onload="filterDetails()">
	...
	<main class="container-fluid">
		...
		<div class="text-right">
			<a href="UpdateBookingServlet" class="btn btn-primary">Update
				Bookings</a>
		</div>
		<form>
		...
		<button type="reset" class="btn btn-primary" onclick="clearFilters()">Clear Filters </button>
		</form>
		...
		function filterDetails(){
			...
			let url = "AllBookingsServletJson";
		...
		function displayDetails(data){				
			...
			if(data.length != 0){
			 for(i = 0; i < data.length; i++){				 
				content+="<tr><td>"+ ++j + "</td><td>" + data[i].ticketId + "</td><td>" + data[i].user.name  + "</td>";
				content+="<td>" + data[i].user.mobileNumber + "</td><td>" + data[i].movie.movieId + "</td>";
